...
const Pricing = () => {
  ...
  const getFeatureValue = (feature, plan) => {
    ...
    if (value === 'advanced') return <span className="text-sm text-[#667eea] font-medium">Advanced</span>;
    ...
    if (value === '24h') return <span className="text-sm text-[#667eea]">24h</span>;
  ...
  return (
    <div className={`min-h-screen ${isDark ? 'bg-gradient-to-b from-gray-950 to-gray-900' : 'bg-gradient-to-b from-gray-50 to-white'}`}>
      ...
      <section className="pt-32 pb-16 px-4 sm:px-6 lg:px-8">
        <div className="max-w-7xl mx-auto text-center">
          ...
          >
            <span className={`font-semibold text-sm uppercase tracking-wider ${isDark ? 'text-[#667eea]' : 'text-[#667eea]'}`}>
              Pricing
            </span>
            <h1 className={`text-3xl sm:text-4xl md:text-5xl font-bold mt-4 mb-6 ${isDark ? 'text-gray-100' : 'text-gray-900'}`}>
              Simple, transparent pricing
            </h1>
            <p className={`text-lg sm:text-xl max-w-3xl mx-auto mb-8 ${isDark ? 'text-gray-300' : 'text-gray-600'}`}>
              Choose the perfect plan for your business. All plans include a 14-day free trial.
            </p>

            {/* Billing Toggle */}
            <div className={`inline-flex items-center p-1 rounded-xl mb-12 ${isDark ? 'bg-gray-800' : 'bg-gray-100'}`}>
              <button
                onClick={() => setBillingCycle('monthly')}
                className={`px-6 py-2 rounded-lg font-medium transition-all ${
                  billingCycle === 'monthly'
                    ? (isDark ? 'bg-gray-700 text-[#667eea] shadow-sm' : 'bg-white text-[#667eea] shadow-sm')
              ...
              >
                Yearly
                <span className={`ml-2 text-xs px-2 py-0.5 rounded-full ${isDark ? 'bg-green-900 text-green-400' : 'bg-green-100 text-green-600'}`}>
                  Save 20%
                </span>
              </button>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 lg:gap-8">
              {plans.map((plan, index) => (
                <motion.div
                  ...
                  className={`relative rounded-2xl shadow-xl transition-all duration-300 flex flex-col h-full ${
                    plan.popular ? 'ring-2 ring-[#667eea]' : ''
                  } ${isDark ? 'bg-gray-900' : 'bg-white'} ${
                    plan.id === 'free' ? 'hover:shadow-2xl hover:scale-105' : ''
                  }`}
                >
                  {plan.popular && (
                    <div className="absolute top-0 left-1/2 transform -translate-x-1/2 -translate-y-1/2 z-10">
                      <span className="bg-gradient-to-r from-[#667eea] to-[#764ba2] text-white px-4 py-1 rounded-full text-sm font-semibold">
                        Most Popular
                      </span>
                    </div>
                  )}

                  <div className="p-6 flex flex-col flex-grow text-start items-start">
                    <h3 className={`text-xl font-bold mb-2 ${isDark ? 'text-gray-100' : 'text-gray-900'}`}>{plan.name}</h3>
                    <p className={`text-sm mb-4 ${isDark ? 'text-gray-300' : 'text-gray-600'}`}>{plan.description}</p>
                    ...
                    {plan.savings > 0 && billingCycle === 'yearly' && (
                      <p className={`text-sm mb-4 ${isDark ? 'text-green-400' : 'text-green-600'}`}>
                      ...
                      <Link
                        ...
                        className={`block w-full py-3 px-4 text-center rounded-xl font-semibold transition-all duration-300 ${
                          plan.popular
                            ...
                            : plan.id === 'free'
                            ? (isDark ? 'bg-gray-800 text-gray-100 hover:bg-gray-700' : 'bg-gray-100 text-gray-900 hover:bg-gray-200')
                            : (isDark ? 'bg-gray-800 text-gray-100 hover:bg-gray-700' : 'bg-gray-900 text-white hover:bg-gray-800')
                        }`}
                      >
                        {plan.cta}
                      </Link>
                  ...
                  <div className={`p-6 border-t ${isDark ? 'border-gray-700' : 'border-gray-100'} flex-grow text-start`}>
                    <p className={`text-sm font-semibold mb-4 ${isDark ? 'text-gray-100' : 'text-gray-900'}`}>What's included:</p>
                    ...
                    {plan.limitations.length > 0 && (
                      <>
                        <p className={`text-sm font-semibold mt-4 mb-2 ${isDark ? 'text-gray-100' : 'text-gray-900'}`}>Limitations:</p>
                        <ul className="space-y-2">
                          {plan.limitations.map((limitation, i) => (
                            <li key={i} className={`flex items-start text-sm ${isDark ? 'text-gray-400' : 'text-gray-500'}`}>
      ...
      <section className={`py-16 px-4 sm:px-6 lg:px-8 ${isDark ? 'bg-gray-900' : 'bg-white'}`}>
        <div className="max-w-7xl mx-auto">
          <h2 className={`text-3xl font-bold text-center mb-12 ${isDark ? 'text-gray-100' : 'text-gray-900'}`}>
          ...
          <div className="overflow-x-auto">
            <table className={`min-w-full divide-y ${isDark ? 'divide-gray-700' : 'divide-gray-200'}`}>
              <thead>
                <tr>
                  <th className={`px-6 py-4 text-left text-sm font-semibold ${isDark ? 'text-gray-100' : 'text-gray-900'}`}>Feature</th>
                  <th className={`px-6 py-4 text-center text-sm font-semibold ${isDark ? 'text-gray-100' : 'text-gray-900'}`}>Free</th>
                  <th className={`px-6 py-4 text-center text-sm font-semibold ${isDark ? 'text-gray-100' : 'text-gray-900'}`}>Starter</th>
                  <th className={`px-6 py-4 text-center text-sm font-semibold ${isDark ? 'text-gray-100' : 'text-gray-900'}`}>Professional</th>
                  <th className={`px-6 py-4 text-center text-sm font-semibold ${isDark ? 'text-gray-100' : 'text-gray-900'}`}>Enterprise</th>
                </tr>
              </thead>
                ...
                {features.map((category, idx) => (
                  <React.Fragment key={idx}>
                    <tr className={isDark ? 'bg-gray-800' : 'bg-gray-50'}>
                      <td colSpan="5" className={`px-6 py-3 text-sm font-semibold ${isDark ? 'text-gray-100' : 'text-gray-900'}`}>
                        {category.category}
                      </td>
                    </tr>
                    {category.items.map((item, i) => (
                      <tr key={i} className={isDark ? 'hover:bg-gray-800' : 'hover:bg-gray-50'}>
                        <td className={`px-6 py-4 text-sm ${isDark ? 'text-gray-300' : 'text-gray-700'}`}>{item.name}</td>
        ...
        <div className="max-w-3xl mx-auto">
          ...
          <div className="space-y-4">
            {faqs.map((faq, index) => (
              <div
                key={index}
                className={`rounded-xl shadow-sm hover:shadow-md transition-shadow p-6 ${isDark ? 'bg-gray-900' : 'bg-white'}`}
              >
                <h3 className={`text-lg font-semibold mb-2 ${isDark ? 'text-gray-100' : 'text-gray-900'}`}>{faq.question}</h3>
      ...
      {/* CTA Section */}
      <section className="py-16 px-4 sm:px-6 lg:px-8 bg-gradient-to-r from-[#667eea] to-[#764ba2]">
        <div className="max-w-4xl mx-auto text-center">
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <Link
              to="/signup"
              className="px-8 py-4 bg-white text-[#667eea] rounded-xl font-semibold hover:shadow-2xl hover:scale-105 transition-all"
            >
              Start Free Trial
            </Link>
            <Link
      ...
      {showEnterpriseModal && (
        <div className={`fixed inset-0 ${
          ...
          <div className={`rounded-2xl max-w-md w-full p-6 ${isDark ? 'bg-gray-900' : 'bg-white'}`}>
            <h3 className={`text-xl font-bold mb-4 ${isDark ? 'text-gray-100' : 'text-gray-900'}`}>Contact Sales</h3>
            ...
            <form className="space-y-4">
              <div>
                ...
                <input
                  type="text"
                  className={`w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 ${isDark ? 'bg-gray-800 border-gray-600 text-gray-100' : 'bg-white border-gray-300 text-gray-900'}`}
              ...
              <div className="flex gap-3 pt-4">
                <button
                  ...
                  className={`flex-1 px-4 py-2 border rounded-lg ${isDark ? 'border-gray-600 text-gray-300 hover:bg-gray-800' : 'border-gray-300 text-gray-700 hover:bg-gray-50'}`}
                >
                  Cancel
                </button>
                <button
                  type="submit"
                  className="flex-1 px-4 py-2 bg-[#667eea] text-white rounded-lg hover:bg-[#5a67d8] transition-colors"
                >
                  Submit
                </button>
              </div>
